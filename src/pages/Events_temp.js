...
function Events() {
  ...
  const fetchEvents = async (page = 1, reset = false) => {
    try {
      ...
      console.log('📄 Pagination:', { current: response.current_page, total: response.last_page });
      ...
      try {
        ...
        if (response.data.success) {
          ...
          if (bannersData.length > 0) {
            console.log('✅ Setting banners:', bannersData);
            setBanners(bannersData);
            setBannerIndex(0); // Reset to first banner
          } else {
            console.log('⚠️ No banners found in response');
            setBanners([]);
          }
        } else {
